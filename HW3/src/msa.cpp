...
int ** scoring_prefix(int start_x, int end_x, int start_y, int end_y, int start_z, int end_z){
    ...
    for(int j = 1; j <= n; j++)
        score_0[j][0] = score_0[j-1][0] + delta('-', y[start_y + j - 1], '-');
    for(int k = 1; k <= p; k++)
        score_0[0][k] = score_0[0][k-1] + delta('-', '-', z[start_z + k - 1]);
    for(int j = 1; j <= n; j++){
        for(int k = 1; k <= p; k++){
            face_jk = score_0[j-1][k-1] + delta('-', y[start_y + j - 1], z[start_z + k - 1]); //3
            edge_j = score_0[j-1][k] + delta('-', y[start_y + j - 1], '-'); //6
            edge_k = score_0[j][k-1] + delta('-', '-', z[start_z + k - 1]); //7
            if(face_jk >= edge_j && face_jk >= edge_k){
                score_0[j][k] = face_jk;
            }
            else if(edge_j >= face_jk && edge_j >= edge_k){
    ...
    for(int i = 1; i <= m; i++){
        score_1[0][0] = score_0[0][0] + delta(x[start_x + i - 1], '-', '-');
        for(int j = 1; j <= n; j++){
            face_ij = score_0[j-1][0] + delta(x[start_x + i - 1], y[start_y + j - 1], '-');
            edge_i = score_0[j][0] + delta(x[start_x + i - 1], '-', '-');
            edge_j = score_1[j-1][0] + delta('-', y[start_y + j - 1], '-');
            if(face_ij >= edge_i && face_ij >= edge_j){
                score_1[j][0] = face_ij;
            }
            else if(edge_i >= face_ij && edge_i >= edge_j){
        ...
        for(int k = 1; k <= p; k++){
            face_ki = score_0[0][k-1] + delta(x[start_x + i - 1], '-', z[start_z + k - 1]);
            edge_k = score_1[0][k-1] + delta('-', '-', z[start_z + k - 1]);
            edge_i = score_0[0][k] + delta(x[start_x + i - 1], '-', '-');
        ...
        for(int j = 1; j <= n; j++){
            for(int k = 1; k <= p; k++){
                cube_diag = score_0[j-1][k-1] + delta(x[start_x + i - 1], y[start_y + j - 1], z[start_z + k - 1]);
                face_ij = score_0[j-1][k] + delta(x[start_x + i - 1], y[start_y + j - 1], '-');
                face_jk = score_1[j-1][k-1] + delta('-', y[start_y + j - 1], z[start_z + k - 1]);
                face_ki = score_0[j][k-1] + delta(x[start_x + i - 1], '-', z[start_z + k - 1]);
                edge_i = score_0[j][k] + delta(x[start_x + i - 1], '-', '-');
                edge_j = score_1[j-1][k] + delta('-', y[start_y + j - 1], '-');
                edge_k = score_1[j][k-1] + delta('-', '-', z[start_z + k - 1]);
...
int ** scoring_suffix(int start_x, int end_x, int start_y, int end_y, int start_z, int end_z){
    int m, n, p, cube_diag, face_ij, face_jk, face_ki, edge_i, edge_j, edge_k;
    m = end_x - start_x;
    n = end_y - start_y;
    p = end_z - start_z;
    ...
    for(int j = 1; j <= n; j++)
        score_0[j][0] = score_0[j-1][0] + delta('-', y[end_y - j], '-');
    for(int k = 1; k <= p; k++)
        score_0[0][k] = score_0[0][k-1] + delta('-', '-', z[end_z - k]);
    for(int j = 1; j <= n; j++){
        for(int k = 1; k <= p; k++){
            face_jk = score_0[j-1][k-1] + delta('-', y[end_y - j], z[end_z - k]); //3
            edge_j = score_0[j-1][k] + delta('-', y[end_y - j], '-'); //6
            edge_k = score_0[j][k-1] + delta('-', '-', z[end_z - k]); //7
            if(face_jk >= edge_j && face_jk >= edge_k){
                score_0[j][k] = face_jk;
            }
            else if(edge_j >= face_jk && edge_j >= edge_k){
    ...
    for(int i = 1; i <= m; i++){
        score_1[0][0] = score_0[0][0] + delta(x[end_x - i], '-', '-');
        for(int j = 1; j <= n; j++){
            face_ij = score_0[j-1][0] + delta(x[end_x - i], y[end_y - j], '-');
            edge_i = score_0[j][0] + delta(x[end_x - i], '-', '-');
            edge_j = score_1[j-1][0] + delta('-', y[end_y - j], '-');
            if(face_ij >= edge_i && face_ij >= edge_j){
                score_1[j][0] = face_ij;
            }
            else if(edge_i >= face_ij && edge_i >= edge_j){
        ...
        for(int k = 1; k <= p; k++){
            face_ki = score_0[0][k-1] + delta(x[end_x - i], '-', z[end_z - k]);
            edge_k = score_1[0][k-1] + delta('-', '-', z[end_z - k]);
            edge_i = score_0[0][k] + delta(x[end_x - i], '-', '-');
            if(face_ki >= edge_k && face_ki >= edge_i){
                score_1[0][k] = face_ki;
            }
            else if(edge_k >= face_ki && edge_k >= edge_i){
        ...
        for(int j = 1; j <= n; j++){
            for(int k = 1; k <= p; k++){
                cube_diag = score_0[j-1][k-1] + delta(x[end_x - i], y[end_y - j], z[end_z - k]);
                face_ij = score_0[j-1][k] + delta(x[end_x - i], y[end_y - j], '-');
                face_jk = score_1[j-1][k-1] + delta('-', y[end_y - j], z[end_z - k]);
                face_ki = score_0[j][k-1] + delta(x[end_x - i], '-', z[end_z - k]);
                edge_i = score_0[j][k] + delta(x[end_x - i], '-', '-');
                edge_j = score_1[j-1][k] + delta('-', y[end_y - j], '-');
                edge_k = score_1[j][k-1] + delta('-', '-', z[end_z - k]);
                int cur_max = cube_diag;
                int temp[7] = {cube_diag, face_ij, face_jk, face_ki, edge_i, edge_j, edge_k};
                for(int l = 1; l < 7; l++){
                    if(temp[l] > cur_max){
...
Result * optimalAlignment3D(int start_x, int end_x, int start_y, int end_y, int start_z, int end_z){
    int m, n, p;
    m = end_x - start_x;
    n = end_y - start_y;
    p = end_z - start_z;
    ...
    int ** pre = scoring_prefix(start_x, start_x + int(m / 2), start_y, end_y, start_z, end_z);
    int ** suf = scoring_suffix(start_x + int(m / 2), end_x, start_y, end_y, start_z, end_z);
    ...
    res1_ = optimalAlignment3D(start_x, start_x + int(m / 2), start_y, y_section, start_z, z_section);
    res2_ = optimalAlignment3D(start_x + int(m / 2), end_x, y_section, end_y, z_section, end_z);
    res = new Result();
...
int main(){
    ...
    clock_t start = clock();
    Result * res2 = optimalAlignment3D(0, x.length(), 0, y.length(), 0, z.length());
    clock_t end = clock();
    ...
    double run_time = ((end - start)/CLOCKS_PER_SEC)/60;
    cout<<"Runtime (minutes): "<<run_time<<endl;
    ...
    outfile<<"Runtime (minutes): "<<run_time<<endl;
